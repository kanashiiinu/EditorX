...
ColorWidget *ColorWidget::instance = nullptr;
...
void ColorWidget::set_color(const QColor &color)
{
  auto red = color.red();
  ...
  QString strGreen = tr("%1").arg(green & 0xFF, 2, 16, QChar('0'));
  QString strBlue = tr("%1").arg(blue & 0xFF, 2, 16, QChar('0'));
  auto color_decimal_val = tr("%1, %2, %3").arg(red).arg(green).arg(blue);
  auto color_hex_val = tr("#%1%2%3").arg(strRed.toUpper()).arg(strGreen.toUpper()).arg(strBlue.toUpper());
  txtRgb->setText(color_decimal_val);
  txtWeb->setText(color_hex_val);

  double gray = (0.299 * color.red() + 0.587 * color.green() + 0.114 * color.blue()) / 255;
  QColor textColor = gray > 0.5 ? Qt::black : Qt::white;

  QString str = tr("background:rgb(%1);color:%2").arg(color_decimal_val).arg(textColor.name());
  labColor->setStyleSheet(str);
}
