...
{
  Q_UNUSED(fun);
//  "for(const auto& item:rng$){ if(params[0]$(item){var$.push_back(item);} }";
  return front::insert_place_hold(R"(for(const auto& item:{})\{ if({}(item))\{{}.push_back(item);\} \})", {rng, params[0], var});
}
...
void from_transform_filter_fold(Sentence_0 const &sentence)
{
...
//  std::cout << std::endl;
  ...
  if (fun_name == "transform") {
    std::cout << transform_op(sentence.name, range, fun_name, params);
  } else if (fun_name == "filter")  {
    std::cout << filter_op(sentence.name, range, fun_name, params);
  } else if (fun_name == "foldl")  {
    std::cout << foldl_op(sentence.name, range, fun_name, params);
  } else if (fun_name == "foldr") {
    std::cout << foldr_op(sentence.name, range, fun_name, params);
  }
  std::cout << std::endl;
}
...
std::string translate(std::string const &head)
{
  ...
  if (sentence.what.front() == '(' and sentence.what.back() == ')')
  {
    // is fun
...
//  res = xs | foldr(f, 0) => res = std::accumulate(std::rbegin(xs), std::rend(xs), 0, f);
  ...
  front::stl::from_transform_filter_fold(front::to_sentence(stl_transform));
